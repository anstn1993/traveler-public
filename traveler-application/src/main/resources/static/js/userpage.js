...
window.addEventListener("load", function () {
    ...
    followingCountBtn.onclick = function () {
        ...
        followListResource = fetchFollowList("followings", userId, getFollowingListUri, followItemTemplate, itemBox);
    }
    //팔로워 개수 클릭 핸들러
    followerCountBtn.onclick = function () {
        createFollowModal(body, "팔로워");
        itemBox = document.querySelector(".follow-list-modal .main");
        moreItem.classList.add("follower");
        moreItem.classList.remove("following");
        itemBox.append(moreItem);//더 보기 아이템 추가(클래스에 off가 존재하기 때문에 화면에 보이지 않는 상태다.)
        followListResource = fetchFollowList("followers", userId, getFollowerListUri, followItemTemplate, itemBox);
    }

    //더 보기 버튼 클릭 핸들러
    moreItem.onclick = function (event) {
        if (moreItem.classList.contains("following")) {
            followListResource = fetchFollowList("followings", userId, followListResource._links.next.href, followItemTemplate, itemBox);
        } else {
            followListResource = fetchFollowList("followers", userId, followListResource._links.next.href, followItemTemplate, itemBox);
        }
...
//팔로우 요청 전송 함수
function sendFollowRequest(userId, followBtn, unfollowBtn) {
    $.ajax({
        type: "POST",
        url: "/users/followings",
        data: {
            "followedAccountId": userId
        }
    }).done(function (res) {
        console.log(res);
...
//언팔로우 요청 전송 함수
function sendUnfollowRequest(userId, followBtn, unfollowBtn) {
    $.ajax({
        type: "DELETE",
...
function fetchFollowList(followingsOrFollowers, userId, getFollowListUri, followItemTemplate, itemBox) {
    $.ajax({
        type: "GET",
        url: "/users/" + userId + "/" + followingsOrFollowers,
        data: {
            "getFollowListUrl": getFollowListUri
        }
    }).done(function (res) {
        followListResource = res;//서버의 응답 리소스
...
//팔로우 모달 창 생성
function createFollowModal(body, modalTitle) {
