...
public class AccountController {
    ...
    //존재하는 사용자인 경우 인증 코드를 생성하여 사용자 메일로 전송하고 세션에 인증코드와 사용자 아이디를 세션에 저장하는 핸들러
    @PostMapping("/find-username")
    public String createAuthCodeAndSendMail(@SessionAttribute(required = false) SessionAccount account,
        ...
        if (targetAccountOpt.isEmpty()) {//사용자 fetch가 되지 않은 경우
            redirectAttributes.addAttribute("message", "이름과 이메일이 잘못되었습니다.");
        ...
        try {
            ...
            //인증 요청 페이지에서
            return "redirect:/find-username/authenticate";
        } catch (Exception e) {
            e.printStackTrace();
    ...
    @GetMapping("/find-username/authenticate")
    ...
    @PostMapping("/find-username/authenticate")
    public ResponseEntity authenticateUserForUsername(@SessionAttribute(required = false) String authCode,
        ...
        if (!authCode.equals(code)) {//사용자가 입력한 코드와 인증코드가 일치하지 않는 경우
            ...
            session.removeAttribute("authCode");//인증 코드 세션에서 제거
    ...
    //사용자 인증 페이지에서 벗어나거나 새로고침을 하는 경우 인증 번호를 세션에서 삭제하기 위한 핸들러
