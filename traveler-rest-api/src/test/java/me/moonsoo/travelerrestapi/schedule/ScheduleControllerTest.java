...
class ScheduleControllerTest extends BaseControllerTest {
    ...
    public void createScheduleMethod() {
        ...
        String password = "1111";
        account = createAccount(email, password, 0);
        Schedule schedule = createSchedule(account, 0, 3, 3);
        assertAll(
    ...
    //일정 게시물 생성 메소드
    public Schedule createSchedule(Account account, int index, int locationCount, int detailCount) {
        Schedule schedule = Schedule.builder()
                .account(account)
                .title("일정" + index)
                .scope(Scope.ALL)
